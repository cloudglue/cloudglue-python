...
class AddCollectionFile(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="User metadata stored on the file (merged into the file's metadata and included in the indexed metadata doc). Only supported when adding to metadata collections; other collection types take metadata via the Files endpoints.")
    __properties: ClassVar[List[str]] = ["segmentation_id", "segmentation_config", "file_id", "url", "thumbnails_config", "metadata"]
